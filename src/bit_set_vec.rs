...
impl IntoIterator for BitSetVec {
...
impl DoubleEndedIterator for BitSetIter {
            ...
            if let Some(new_n) = n.checked_sub(self.inner.0[word_index].count_ones()) {
                ...
                loop {
                    ...
                    if let Some(new_n) = n.checked_sub(leading_ones) {
                        ...
                        word <<= n + 1;
                        let r = (WORD_BITS - (shift + n + 1)) + (word_index as u32 * (WORD_BITS));
    ...
    {
        let mut accum = init;
        let mut index = self.inner.0.len() as u32;
...
pub mod tests {
    ...
    fn test_sum() {
        ...
        assert_eq!(
            BitSetVec::from_first_n(256).into_iter().sum::<u32>(),
            (0..256).sum::<u32>()
        );
    }
