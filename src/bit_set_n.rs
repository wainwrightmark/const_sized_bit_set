use core::{iter::FusedIterator, u16};
...
impl BitSet16 {
    ...
    pub const MAX_COUNT: u32 = u16::BITS;
    ...
    /// ```
    /// use const_sized_bit_set::BitSet16;
    /// assert_eq!(BitSet16::from_inner(0b1101).with_inserted(1).inner(), 0b1111);
    /// ```
    #[must_use]
    #[inline]
    ...
    /// ```
    /// use const_sized_bit_set::BitSet16;
    /// assert_eq!(BitSet16::from_inner(0b0010).with_inserted(2), BitSet16::from_inner(0b0110));
    /// ```
    #[must_use]
    #[inline]
    ...
    /// ```
    /// use const_sized_bit_set::BitSet16;
    /// assert_eq!(BitSet16::from_inner(0b0110).with_removed(2), BitSet16::from_inner(0b0010));
    /// ```
    #[must_use]
    #[inline]
    ...
    /// ```
    /// use const_sized_bit_set::BitSet16;
    /// assert_eq!(BitSet16::from_inner(0b0110).with_bit_set(3, true).with_bit_set(2, false), BitSet16::from_inner(0b1010));
    /// ```
    #[must_use]
    #[inline]
    ...
    /// ```
    /// use const_sized_bit_set::BitSet16;
    /// assert_eq!(BitSet16::from_inner(0b10), BitSet16::from_inner(0b01).with_bits_swapped(0,1))
    /// ```
    #[must_use]
    #[inline]
    ...
    /// ```
    /// use const_sized_bit_set::BitSet16;
    /// assert_eq!(BitSet16::from_inner(0b001), BitSet16::from_inner(0b101).with_intersection(&BitSet16::from_inner(0b011)))
    /// ```
    #[must_use]
    ...
    /// ```
    /// use const_sized_bit_set::BitSet16;
    /// assert_eq!(BitSet16::from_inner(0b111), BitSet16::from_inner(0b101).with_union(&BitSet16::from_inner(0b011)))
    /// ```
    #[allow(dead_code)]
    #[must_use]
    ...
    /// ```
    /// use const_sized_bit_set::BitSet16;
    /// assert_eq!(BitSet16::from_inner(0b0100), BitSet16::from_inner(0b0111).with_except(&BitSet16::from_inner(0b0011)))
    /// ```
    #[allow(dead_code)]
    #[must_use]
    ...
    pub const fn pop(&mut self) -> Option<u32> {
        ...
        let t = self.0 & (0u16.wrapping_sub(self.0));
        self.0 ^= t;
    ...
    /// ```
    /// use const_sized_bit_set::*;
    /// assert_eq!(BitSet16::from_first_n(8).min_set_by_key(|x|x % 2), BitSet16::from_inner(0b01010101));
    /// ```
    #[must_use]
...
impl FromIterator<u32> for BitSet16 {
...
pub struct AdjacencyIter(BitSet16);

impl ExactSizeIterator for AdjacencyIter {}

impl FusedIterator for AdjacencyIter {}
...
impl Iterator for AdjacencyIter {
        ...
        S: core::iter::Sum<Self::Item>,
    {
        ...
        if self.0 == BitSet16::ALL {
            const MAX_COUNT: u32 = (BitSet16::MAX_COUNT * (BitSet16::MAX_COUNT - 1)) / 2;
            total += MAX_COUNT;
        } else {
            while value != 0 {
...
impl DoubleEndedIterator for AdjacencyIter {
    #[inline]
    ...
    fn nth_back(&mut self, n: usize) -> Option<Self::Item> {
        ...
        loop {
            ...
            if let Some(new_n) = n.checked_sub(leading_ones) {
                ...
                self.0 .0 <<= n + 1;
                let r = BitSet16::MAX_COUNT - (shift + n + 1);
    ...
    {
        let mut accum = init;
        let mut offset = BitSet16::MAX_COUNT;

        //special case - prevents overflow when right shifting

        if self.0 .0 == u16::MAX {
            //special case - prevents overflow when right shifting
            for v in (0..(BitSet16::MAX_COUNT)).rev() {
                accum = f(accum, v);
            }
        } else {
            while self.0 .0 != 0 {
